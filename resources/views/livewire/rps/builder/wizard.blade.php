<div>
    <div class="page-header d-print-none">
        <div class="row align-items-center">
            <div class="col">
                <h2 class="page-title">
                    {{ $isCreating ? 'Buat RPS Baru' : 'Edit RPS' }}
                </h2>
                @if($rps && $rps->exists)
                    <div class="text-secondary mt-1">
                        {{ $rps->mataKuliah->name ?? '-' }} - {{ $rps->semester->name ?? '-' }}
                    </div>
                @endif
            </div>
            <div class="col-auto ms-auto d-flex gap-2">
                <button wire:click="saveDraft" class="btn btn-outline-primary" wire:loading.attr="disabled">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke="none" d="M0 0h24v24H0z"/><path d="M6 4h10l4 4v10a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2"/><path d="M12 14m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0"/><path d="M14 4l0 4h-6l0 -4"/></svg>
                    Simpan Draft
                </button>
                <div class="d-flex align-items-center">
                    <div class="progress" style="width: 150px; height: 8px;">
                        <div class="progress-bar bg-primary" style="width: {{ $completionPercentage }}%" role="progressbar"></div>
                    </div>
                    <span class="ms-2 text-secondary small">{{ $completionPercentage }}%</span>
                </div>
            </div>
        </div>
    </div>

    <div class="card mt-3">
        <div class="card-body">
            <div class="steps steps-counter steps-primary mb-4">
                @for($i = 1; $i <= 8; $i++)
                    @php
                        $status = $this->getStepStatus($i);
                        $label = $this->getStepLabel($i);
                    @endphp
                    <a href="#"
                       wire:click.prevent="goToStep({{ $i }})"
                       @class([
                           'step-item',
                           'active' => $status === 'current',
                       ])
                       style="@if($status === 'completed') color: var(--tblr-primary); cursor: pointer; @elseif($status === 'pending') pointer-events: none; opacity: 0.5; @endif text-decoration: none;">
                        <div class="step-marker">
                            @if($status === 'completed')
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke="none" d="M0 0h24v24H0z"/><path d="M5 12l5 5l10 -10"/></svg>
                            @else
                                {{ $i }}
                            @endif
                        </div>
                        <span class="step-label">{{ $label }}</span>
                    </a>
                @endfor
            </div>

            @if(session()->has('message'))
                <div class="alert alert-success alert-dismissible" role="alert">
                    {{ session('message') }}
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            @endif

            @if(session()->has('step_errors'))
                <div class="alert alert-danger alert-dismissible" role="alert">
                    <ul class="mb-0">
                        @foreach(session('step_errors') as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            @endif

            <div class="hr-text hr-text-center hr-text-spaceless mt-3 mb-4">
                <span class="badge bg-primary-lt">Step {{ $currentStep }} / 8</span>
            </div>

            <div class="step-content">
                @if($currentStep >= 1 && $rps)
                    @livewire('rps.builder.step1-informasi-mk', ['rpsId' => $rps->id ?? null], key('step1-'.$rps->id))
                @endif
                @if($currentStep >= 2 && $rps && $rps->exists)
                    @livewire('rps.builder.step2-pilih-cpl', ['rpsId' => $rps->id], key('step2-'.$rps->id))
                @endif
                @if($currentStep >= 3 && $rps && $rps->exists)
                    @livewire('rps.builder.step3-cpmk', ['rpsId' => $rps->id], key('step3-'.$rps->id))
                @endif
                @if($currentStep >= 4 && $rps && $rps->exists)
                    @livewire('rps.builder.step4-sub-cpmk', ['rpsId' => $rps->id], key('step4-'.$rps->id))
                @endif
                @if($currentStep >= 5 && $rps && $rps->exists)
                    @livewire('rps.builder.step5-materi', ['rpsId' => $rps->id], key('step5-'.$rps->id))
                @endif
                @if($currentStep >= 6 && $rps && $rps->exists)
                    @livewire('rps.builder.step6-metode', ['rpsId' => $rps->id], key('step6-'.$rps->id))
                @endif
                @if($currentStep >= 7 && $rps && $rps->exists)
                    @livewire('rps.builder.step7-assessment', ['rpsId' => $rps->id], key('step7-'.$rps->id))
                @endif
                @if($currentStep >= 8 && $rps && $rps->exists)
                    @livewire('rps.builder.step8-review', ['rpsId' => $rps->id], key('step8-'.$rps->id))
                @endif
            </div>

            <div class="d-flex justify-content-between mt-4 pt-3 border-top">
                <button wire:click="prevStep" class="btn btn-outline-secondary" @if($currentStep <= 1) disabled @endif>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke="none" d="M0 0h24v24H0z"/><path d="M15 6l-6 6l6 6"/></svg>
                    Sebelumnya
                </button>

                @if($currentStep < 8)
                    <button wire:click="nextStep" class="btn btn-primary">
                        Selanjutnya
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke="none" d="M0 0h24v24H0z"/><path d="M9 6l6 6l-6 6"/></svg>
                    </button>
                @else
                    <span></span>
                @endif
            </div>
        </div>
    </div>
</div>
