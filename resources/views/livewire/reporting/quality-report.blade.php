<div>
    ...
    @if($data)
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                @if(empty($data['prodiAverages']))
                    <div class="text-center text-secondary py-4">Belum ada data review untuk filter yang dipilih</div>
                @else
                    @php $maxAvgScore = max(array_column($data['prodiAverages'], 'avgScore')) ?: 1; @endphp
                    @foreach($data['prodiAverages'] as $pa)
                        @php $barPct = ($pa['avgScore'] / $maxAvgScore) * 100; @endphp
                        <div class="mb-3">
