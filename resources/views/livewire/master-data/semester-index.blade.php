<x-layouts.app title="Data Semester">
    <div class="card">
        ...
        <div class="table-responsive">
            <table class="table table-vcenter card-table table-striped">
                ...
                <tbody>
                    @foreach($this->semesterList() as $item)
                        <tr>
                            <td>{{ $item->name }}</td>
                    ...
                    <form wire:submit="save">
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <input type="text" wire:model="name" class="form-control @error('name') is-invalid @enderror" placeholder="Semester Ganjil 2024/2025">
                                @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="mb-3">
                                <label class="form-label required">Tipe</label>
                                <select wire:model="tipe" class="form-select @error('tipe') is-invalid @enderror">
                                    <option value="">Pilih Tipe</option>
                                    @foreach($this->tipeOptions() as $t)
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" wire:model="tahun_akademik" class="form-control @error('tahun_akademik') is-invalid @enderror" placeholder="2024/2025">
                                @error('tahun_akademik') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label required">Tanggal Mulai</label>
                                    <input type="date" wire:model="tanggal_mulai" class="form-control @error('tanggal_mulai') is-invalid @enderror">
                                    @error('tanggal_mulai') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label required">Tanggal Selesai</label>
                                    <input type="date" wire:model="tanggal_selesai" class="form-control @error('tanggal_selesai') is-invalid @enderror">
